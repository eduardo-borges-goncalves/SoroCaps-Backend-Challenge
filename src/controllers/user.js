"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.deleteUser = exports.updateUser = exports.postUser = exports.getUser = exports.getUsers = void 0;
var user_1 = require("../models/user");
var bcrypt = require("bcrypt");
var getUsers = function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var users, error_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, user_1.UserModel.findAll({
                        attributes: ['id', 'name', 'userLogin']
                    })];
            case 1:
                users = _a.sent();
                return [2 /*return*/, res.status(200).json({
                        data: users
                    })];
            case 2:
                error_1 = _a.sent();
                return [2 /*return*/, next(new Error(error_1))];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getUsers = getUsers;
var getUser = function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var user, error_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                return [4 /*yield*/, findUser(req.params.id)];
            case 1:
                user = _a.sent();
                if (!user) {
                    res.status(400).send({ error: "Usuário não encontrado" });
                }
                return [2 /*return*/, res.status(200).json({
                        data: { id: user.id, name: user.name, userLogin: user.userLogin }
                    })];
            case 2:
                error_2 = _a.sent();
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getUser = getUser;
var postUser = function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, name_1, userLogin, password, hashPassword, user, error_3;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 3, , 4]);
                _a = req.body, name_1 = _a.name, userLogin = _a.userLogin, password = _a.password;
                if (!name_1 || !userLogin || !password) {
                    res.status(400).send({ error: "Característica necessária à criação não informada" });
                }
                return [4 /*yield*/, bcrypt.hash(password, 16)];
            case 1:
                hashPassword = _b.sent();
                return [4 /*yield*/, user_1.UserModel.create({
                        name: name_1,
                        userLogin: userLogin,
                        password: hashPassword
                    })];
            case 2:
                user = _b.sent();
                return [2 /*return*/, res.status(201).json({
                        data: { id: user.id, name: user.name, userLogin: user.userLogin }
                    })];
            case 3:
                error_3 = _b.sent();
                return [2 /*return*/, next(new Error(error_3))];
            case 4: return [2 /*return*/];
        }
    });
}); };
exports.postUser = postUser;
var updateUser = function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, name_2, userLogin, password, user, hashPassword, updatedUser, error_4;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _b.trys.push([0, 5, , 6]);
                _a = req.body, name_2 = _a.name, userLogin = _a.userLogin, password = _a.password;
                return [4 /*yield*/, findUser(req.params.id)];
            case 1:
                user = _b.sent();
                if (!user) {
                    res.status(400).send({ error: "Usuário não encontrado" });
                }
                return [4 /*yield*/, bcrypt.hash(password, 8)];
            case 2:
                hashPassword = _b.sent();
                return [4 /*yield*/, user_1.UserModel.update({
                        name: name_2 || user.name,
                        userLogin: userLogin || user.userLogin,
                        password: hashPassword || user.password
                    }, {
                        where: { id: user.id },
                        returning: true,
                        plain: true
                    })];
            case 3:
                _b.sent();
                return [4 /*yield*/, findUser(req.params.id)];
            case 4:
                updatedUser = _b.sent();
                return [2 /*return*/, res.status(201).json({
                        data: {
                            id: updatedUser.id,
                            name: updatedUser.name,
                            userLogin: updatedUser.userLogin
                        }
                    })];
            case 5:
                error_4 = _b.sent();
                return [2 /*return*/, next(new Error(error_4))];
            case 6: return [2 /*return*/];
        }
    });
}); };
exports.updateUser = updateUser;
var deleteUser = function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var user;
    return __generator(this, function (_a) {
        try {
            user = findUser(req.params.id);
            if (!user) {
                res.status(400).send({ error: "Usuário não encontrado" });
            }
            return [2 /*return*/, res.status(204).send({})];
        }
        catch (error) {
            return [2 /*return*/, next(new Error(error))];
        }
        return [2 /*return*/];
    });
}); };
exports.deleteUser = deleteUser;
var findUser = function (id) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, user_1.UserModel.findOne({
                    where: { id: id }
                })];
            case 1: return [2 /*return*/, _a.sent()];
        }
    });
}); };
